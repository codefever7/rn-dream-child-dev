...
const PrivacyPolicyScreens = ({ navigation }) => {
  const userSelector = useSelector((state) => state.user);
  const { appDetail, loading } = userSelector;
  const dispatch = useDispatch();
  const { width } = useWindowDimensions();

  const privacy_policy = useMemo(() => {
    if (!appDetail) return;
    const privacyPolicy = fromBase64(appDetail?.AppDetail?.privacy_policy);
    ...
    navigation.setOptions({
      ...
      headerLeft: () => (
        <View style={s.headerRow}>
          ...
          />
          <TextView text={'Privacy Policy'} type={'body-two'} style={s.headerText} />
        </View>
      ),
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        <View style={s.mainroot}>
            ...
            <View style={s.imgview2}>
              <ImageBackground source={require('../../assets/img/policybg.png')} resizeMode='contain' style={s.fondimg}>
                <View style={s.rowlist}>
                  <Image
                    source={require('../../assets/img/partnership.png')}
                    resizeMode='contain'
                    style={s.policyimg}
                  />
                  <View style={s.textcontain}>
                    <TextView text={'Privacy Policy'} type={'body-head'} style={s.blacktext} />
                    <TextView
                      text={
          ...
          <View style={s.textlist}>
            <RenderHTML contentWidth={width} source={{ html: privacy_policy }} tagsStyles={s.tagsStyles} />
          </View>
        </View>
      </ScrollView>
    </View>
